...
impl ImageData {
    ...
    pub(crate) fn load() -> Result<Self, Box<dyn Error>> {
        ...
        let ret = if data_path.exists() {
            ...
            let image_data = serde_json::from_str(&data_contents)?;
            Ok(image_data)
        } else {
            Ok(Self {
                urls: Vec::new(),
    ...
    pub(crate) fn store(&self) -> Result<(), Box<dyn Error>> {
        ...
        let data_contents = serde_json::to_string(self)?;
        // Create the parent directory if needed
        if let Some(parent) = data_path.parent() {
            fs::create_dir_all(parent)?;
        }
        let mut file = fs::File::create(data_path)?;
        file.write_all(data_contents.as_bytes())?;
...
pub(crate) fn select_random_image(
    ...
    let use_local_image = rng.gen::<bool>();
    ...
    if use_local_image {
        ...
        if !local_images.is_empty() {
            for _ in 0..10000 {
                ...
                #[expect(clippy::unwrap_used)]
                let image_path = local_images.choose(&mut rng).unwrap().clone();
...
pub(crate) fn get_images(pictures_dir: &Path) -> Result<Vec<PathBuf>, Box<dyn Error>> {
    ...
    for entry in fs::read_dir(pictures_dir)? {
        ...
        let path = entry.path();
        if path.is_dir() {
            let mut other_images = get_images(&path)?;
